...
- 主区与侧栏的透明底色改为独立合成图层；拖动透明度时只更新两个图层的 `opacity`，不再让整窗多段 `color-mix` 渐变随每一帧重新栅格化。
- 壁纸滤镜层显式提升为独立合成层，减少亮度和清晰度调节时对界面正文的连带重绘。
- 修复外观容器的绘制隔离会裁掉上方弹出面板、导致点击“外观”后看不到调节项的问题。

### 测试

- 增加主区/侧栏合成遮罩的默认及高透明度断言，并覆盖外观面板不得启用绘制裁剪的回归检查。

## 1.3.1 — 2026-07-17

### 修复

- 大幅降低任务页整窗渐变和正文增强层的不透明度；透明度滑杆现在真实控制任务背景，而不再额外叠加最高 92% 的浅色遮罩。
- 滑杆输入改为每帧最多应用一次，只更新当前设置组；本地存储改为停止拖动后延迟写入，避免每个像素变化都重算全部变量、切换根节点 class 并同步写盘。
- 外观面板移除实时调节时成本较高的背景模糊，并限制其绘制范围，减少壁纸与面板同时重绘造成的卡顿。

### 测试

- 增加默认与高透明度下的任务页遮罩回归断言，并检查按帧合并与延迟持久化路径。

## 1.3.0 — 2026-07-17

### 新增

- Codex 右下角新增紧凑的实时外观面板，可调界面透明度、背景亮度、背景清晰度，并可分别关闭动态氛围和正文增强。
- 支持在 Codex 内直接更换当前主题壁纸；覆盖按主题 ID 保存在本地浏览器存储中，renderer 重载后仍可恢复，并可一键回到主题仓库当前壁纸。
- Windows 托盘、主题导入、元数据校验和注入 payload 新增 GIF 与 AVIF 支持；GIF 保留原始动画播放。
- 新增非交互的环境粒子与柔光层，并为长正文、代码块、引用和表格提供自适应阅读表面。

### 修复

- early payload 在 `documentElement` 尚未创建时会等待 `DOMContentLoaded` 后再观察 Codex shell，完整 renderer 重载不再因 document-start 时序丢失皮肤。
- 主区、侧栏、任务页、输入框和菜单的表面透明度改为统一变量驱动，亮度与清晰度只作用于壁纸层，不降低正文清晰度。

### 测试

- 增加 GIF/AVIF 元数据、外观设置持久化、动态/阅读开关、控制面板结构和 document-start 重载时序回归测试。
- 在隔离的官方 Store Codex 副本中实测首页、任务正文、实时滑杆、GIF 导入与播放、设置/壁纸跨 renderer 重载恢复。
