...
- 修复用户自定义安装目录后，安装结束启动 PowerShell 时可能因工作目录不存在而报错 `267` 的问题。
- 安装、启动、更新与卸载统一使用显式状态目录；受管运行时从自身安装位置识别状态根目录，不再依赖子进程继承的 `LOCALAPPDATA`。
- 安装器快捷方式和安装后任务使用用户选择的应用目录作为工作目录，并新增“自定义安装目录与状态目录不同”的安装/卸载回归测试。

## 1.4.1 — 2026-07-18

- 新增无需管理员权限的 Windows `Setup.exe`，提供图形化安装、开始菜单入口和系统“已安装的应用”卸载入口。
- 新增托盘与开始菜单“检查更新”：每天最多自动检查一次稳定版，直接读取 Release 固定更新清单，不消耗匿名 REST API 配额；用户确认后下载，并严格校验版本、仓库、下载地址和 SHA-256。
- 卸载器会恢复 Codex 官方外观并移除受管运行时；用户可选择保留个人主题数据，或一并清除当前主题、已保存主题与导入图片。
- 新增 `windows-v*` 标签触发的 GitHub Actions 发布流程，固定并校验 Inno Setup 编译器，构建 EXE、SHA-256 文件与更新清单；配置证书后可自动签名。

## 1.4.0 — 2026-07-18

### 新增

- 右下角「外观」面板新增统一壁纸管理区，可在「哥特虚空」「财神工坊」「红白科幻」「淡绿清岚」四张预置壁纸之间直接切换。
- 保留自定义壁纸上传能力，预置与自定义选择都会按当前主题持久化；「恢复当前主题」仍可回到托盘或主题仓库选定的壁纸。

### 优化

- 四张预置图以原分辨率高质量 JPEG 随 Windows 运行时分发，降低注入载荷，同时避免二次裁切或重绘画面。
- 外观面板扩展为可滚动布局，并为当前选中的预置或自定义壁纸提供明确状态。
- 宽屏壁纸模式下，左侧栏改为独立深色阅读层，增加背景模糊、右侧分隔和清晰选中态；侧栏透明度拥有独立下限，亮色壁纸不再干扰项目与任务列表。

### 测试

- 增加预置壁纸载荷、对象 URL 生命周期、控制区结构与选中态样式的回归检查。
- 增加默认及高透明度下的侧栏遮罩强度回归检查，并在真实首页、任务页验证侧栏结构、文字对比度与横向溢出。

## 1.3.3 — 2026-07-17

### 修复

- 正文增强不再依赖新版 Codex 已移除的任务路由容器，改为跟随当前稳定的对话属性。
- 普通用户消息、助手正文、Markdown、代码块、引用和表格现在都会获得对应强度的局部可读表面，不再通过整页遮罩牺牲壁纸清晰度。

### 测试

- 增加普通消息与助手正文稳定属性的回归断言，并防止正文增强重新退回旧任务容器选择器。
