...
普通安装和使用不需要管理员权限，也不需要接管 WindowsApps 目录。

## 安装

推荐从 [Releases](https://github.com/413162826/Codex-Dream-Skin/releases/latest) 下载 `Codex-Dream-Skin-Windows-*-Setup.exe`，双击完成安装。程序文件可以安装到用户选择的任意目录；主题和运行状态保存在当前用户的本地应用数据目录。安装器会关闭正在运行的 Dream Skin 托盘和 Codex，校验官方 Codex Store 包和 Node.js，部署受管运行时，并在开始菜单注册启动、检查更新和卸载入口。

从源码安装仅用于开发和排障。在 PowerShell 中进入仓库的 `windows` 目录，然后运行：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\install-dream-skin.ps1
```

源码安装器会保存可恢复的外观配置，并初始化本地主题仓库。默认还会创建这些快捷方式：
...
EXE 安装版会在托盘启动时每天最多检查一次最新稳定版，也可以从托盘或开始菜单手动点击“检查更新”。更新器直接读取 GitHub Release 内的固定更新清单，不依赖匿名 REST API 配额。发现新版本后，只有用户确认才会下载；安装包的版本、仓库、下载地址和更新清单中的 SHA-256 全部匹配后才会启动。更新会原子替换受管运行时，当前主题、已保存主题和导入图片不会被删除。

从 `windows-v1.3.3` 或 `windows-v1.4.0` ZIP 安装的用户需要手动下载并运行一次新版 `Setup.exe`。完成这次迁移后，后续版本可使用内置更新器。

源码安装仍可先退出托盘和 Codex，执行 `git pull` 后重新运行安装命令。
...
- 更换 PNG、JPEG、WebP、GIF 或 AVIF 背景图。
- 保存当前主题并从「已保存主题」切换。
- 暂停或继续显示皮肤。
- 重新应用主题，或完整恢复 Codex。
...
Codex 窗口右下角还有一个「外观」按钮，可以在不离开当前任务的情况下实时调节：

- 界面透明度、背景亮度和背景清晰度。
- 动态氛围与正文增强开关；正文增强覆盖普通对话、Markdown、代码块、引用和表格。
- 统一壁纸管理：可直接切换「哥特虚空」「财神工坊」「红白科幻」「淡绿清岚」四张预置壁纸，也可上传 PNG、JPEG、WebP、GIF 或 AVIF 自定义壁纸。

外观数值会保存在当前 Codex 用户配置中；预置或自定义壁纸选择按主题 ID 保存在本地浏览器存储中，重载后仍会恢复。点击「恢复当前主题」会删除这层窗口内覆盖，重新使用托盘或主题仓库中当前选定的壁纸。

## 恢复与卸载

EXE 安装版可以从 Windows“已安装的应用”或开始菜单直接卸载。卸载器会恢复 Codex 外观、关闭 Dream Skin 运行时并移除程序文件；卸载时可以选择保留个人主题数据，或一并删除当前主题、已保存主题和导入图片。
...
| 更新下载缓存 | `%LOCALAPPDATA%\CodexDreamSkin\updates` |
| EXE 安装文件 | `%LOCALAPPDATA%\Programs\CodexDreamSkin` |
